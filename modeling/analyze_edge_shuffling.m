...
load cellreports_review\dtinetwork_edgeshuffling.mat
...
for i = 1:nshuffles
    ...
    xticks([1 2])
    if (i == 4) | (i == 5) | (i == 6), xticklabels(["Rest", "Stimulation"]), else, xticklabels(["", ""]), end
    if (i == 1) | (i == 4), ylabel("timescale (\tau)"), end
    stattext = get_stattext_wilcox(zs(i), ps_correct(i), rs(i), 'horizontal');
    ylim([0.07    0.18])
    sigstar_duodenal({[1 2]}, ps_correct(i), stattext, 0, 0.05);
    
    title(sprintf("%d%%", 100*surrogates(i)))
end
fontsize(gcf, 14, "points")
sgtitle("Shuffling percentage: ")
% saveas(gcf, 'figs\supp_dti_edgeshuffling.png')
% saveas(gcf, 'figs\supp_dti_edgeshuffling.fig')
exportgraphics(gcf, 'cellreports_review\figs\supp_dti_edgeshuffling.jpg')