...
%
% Dependent variables:
%   Rest - Running ACW percent change (for each region vs averaged across regions)
...
calculations.acw_var_x_rois_mice = mice_onehot;
...
save('restvariability_acw0.mat', "calculations")
%% Dependent variables
rundata = dict(run, :);
runacws = rundata.acw;
...
for i = 1:nmice
    ...
    acw_prcchange_x_trials(:, i) = ((i_runacws_ave - i_restacws_ave) ./ i_runacws_ave) .* 100;
end
%% Save percent change
calculations = [];
...
save('rest_run_prcchange_acw0.mat', "calculations")
