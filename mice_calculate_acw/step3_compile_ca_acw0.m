...
for m = 1:length(mice)
    ...
    for i = 1:length(states)
        ...
        if string(s) == string('post_offset')
            for j = 1:nruns
                all_acw.initrest = [all_acw.initrest, data(j).initrest_acw.ca_acw0];
                all_acw.susrest = [all_acw.susrest, data(j).susrest_acw.ca_acw0];
            ...
            for j = 1:nruns
                all_acw.(s) = [all_acw.(s), data(j).([s, '_acw']).ca_acw0];
...
save(fullfile(acwDIR, 'states', 'acw0_ca'), 'acwmat', 'labels', ...
