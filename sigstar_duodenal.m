function varargout=sigstar_duodenal(groups,stats, stattext, nosort, in_offset)
    ...
    for ii=1:length(groups)
        ...
        H(ii,:)=makeSignificanceBar(xlocs(ii,:),thisY,stats(ii), stattext, in_offset);
    end
    %-----------------------------------------------------
...
function H=makeSignificanceBar(x,y,p, stattext, in_offset)
    ...
    % if class(stattext) == string('char')
    %     stattext2 = [stattext, ' ', stars];
    % elseif class(stattext) == string('string')
    %     stattext2 = stattext + " " + string(stars);
    % else
    %     error("WTF BRO!??")
    % end
    ...
    H(2)=text(mean(x(:)),starY,stattext,...
